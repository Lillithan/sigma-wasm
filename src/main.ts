...
import type { Layer, WasmAstar, WasmModule } from './types';
...
const init = async (): Promise<void> => {
  ...
  // wasm-bindgen expects these functions to be available in global scope
  // when __wbg_get_imports() is called internally
  const wasmImports = getWasmImports();
  
  // Make functions available globally for wasm-bindgen
  // The generated code expects these to be in scope when __wbg_get_imports() runs
  // Use bracket notation to assign without type assertions
  ...
  // Initialize wasm-bindgen (no imports parameter needed - functions are global)
  ...
  // Type-safe assignment - wasm-bindgen exports match our WasmModule interface
  if (
    ...
    const wasmModule: WasmModule = {
      ...
      init: (): void => {
        // wasm-bindgen init is called above
      },
...
const getWasmImports = () => {
  ...
  return {
    ...
    js_start_interval_tick(ms: number): void {
      ...
      // Use a recursive requestAnimationFrame pattern instead of setInterval
...
// wasmReadStrFromMemory no longer needed with wasm-bindgen string handling
